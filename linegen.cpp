#include <windows.h>  // Headers needed for OpenGL
...
int* mapInput; //Globally declared pointer used for OpenGL map generation
...
void display() {
	...
	//varibles used for 'pixel' corner positionaing and coloring
	float color = 0;
	int i = 0;
	float BLx = -1.0;
	float BLy = 0.98;
	...
	for (int y = 0; y < 100; y++) {
		for (int x = 0; x < 100; x++) {
			color = (float)mapInput[i] / 9;
			glColor3f(color, color, color);		  //Set RGB color of shape with a float value that represents percentage, ex: glColor3f(0.25, 0.5, 1.0) 
			glVertex2f(BLx, BLy);     // Define vertices in counter-clockwise (CCW) order, starting with bottom left vertex
			glVertex2f(BRx, BRy);
			glVertex2f(TRx, TRy);
			glVertex2f(TLx, TLy);
			BLx += 0.02;
...
int main(int argc, char** argv) {
	...
	while (1) { // Since it's RNG... we'll need a lot to detect even a tiny bug.
		...
		//OpenGL Functions from gl headers
		/************************************************************************************************************************/
		mapInput = map;
		glutInit(&argc, argv);          // Initialize GLUT
		glutCreateWindow("Map");  // Create window with the given title
