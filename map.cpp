...
worldMap::worldMap(unsigned int seedInput,unsigned int worldMapSideInput, unsigned int mapSideInput, unsigned int tileSideInput, 
...
map::map(unsigned int seedInput, worldMap* parentW):
	seed(seedInput),
...
tile::tile(unsigned int seedInput, map* parentMInput):
	seed(seedInput),
...
void genTile(unsigned int seed, double pushCoefficient, int pointY, int pointX, int* map, unsigned int side, unsigned int tileSide, bool diagonal, bool debug){
	...
	// --- k done
	// (unsigned int seed, unsigned int* index, unsigned int* spareIndex, double pushCoefficient, 
   	// int startY, int startX, int endY, int endX, unsigned int* yList, 
    	// unsigned int* xList, int* ySpareList, int* xSpareList,
    	// unsigned int side, unsigned int sideLimit, bool borderLimit, bool diagonal, bool debug)

	// int filler, int detect, int wall, int pointY, int pointX, unsigned int side, 
	// int* map, int* spareMap, bool wallMode, bool replace
	unsigned int pointsX[tileSide*tileSide*3];
	unsigned int pointsY[tileSide*tileSide*3];
	unsigned int index = 0;
	unsigned int spareIndex = 0;
	unsigned int orthogonal[4] = {0,0,0,0}; // N W S E
	int sparePointsX[tileSide*tileSide*3];
	int sparePointsY[tileSide*tileSide*3];
	for(int level = 0; level<max;level++){
		...
		if(checkDir[0] > level+1 && checkDir[3] <= level+1 && checkDir[1] <= level+1){ // NW corner - single
			// ---
			randLine(rand(), &index, &spareIndex, pushCoefficient, 0, shift, 
			shift, 0, pointsY, pointsX, sparePointsY, sparePointsX, tileSide, 
			...
			if(checkDir[3] > level+1 || checkDir[5] > level+1){ // Since there's more standard shapes,
				...
				if(checkDir[3] > level+1 && checkDir[5] > level+1){ // N & W & E
					...
					shift, shift, shift, (tileSide-1)-shift, 
					...
					// ---
					randLine(rand(), &index, &spareIndex, pushCoefficient, 
					shift, 0, shift, (tileSide-1)-shift, 
					pointsY, pointsX, sparePointsY, sparePointsX, tileSide, 
					0, true, diagonal, debug); //
					for(int i = 0; i<index;++i){
						spareTileMap[(pointsY[i]*tileSide) + pointsX[i] ] = 2;
		...
		if(checkDir[2] > level+1 && checkDir[5] <= level+1 && checkDir[1] <= level+1){ // NE corner - single
			...
			if(checkDir[1] > level+1 || checkDir[7] > level+1){ // Since there's more standard shapes,
				...
				if(checkDir[1] > level+1 && checkDir[7] > level+1){ // N & E & S
					// ---
					randLine(rand(), &index, &spareIndex, pushCoefficient, 
					shift, (tileSide-1)-shift, (tileSide-1)-shift, (tileSide-1)-shift, 
					pointsY, pointsX, sparePointsY, sparePointsX, tileSide, 
					0, true, diagonal, debug);
					for(int i = 0; i<index;++i){
						spareTileMap[(pointsY[i]*tileSide) + pointsX[i] ] = 2;
					...
					// ---
					randLine(rand(), &index, &spareIndex, pushCoefficient, 
					shift, (tileSide-1)-shift, (tileSide-1), (tileSide-1)-shift, 
					pointsY, pointsX, sparePointsY, sparePointsX, tileSide, 
					0, true, diagonal, debug);
					for(int i = 0; i<index;++i){
						spareTileMap[(pointsY[i]*tileSide) + pointsX[i] ] = 2;
					...
					// ---
					randLine(rand(), &index, &spareIndex, pushCoefficient, 
					0, (tileSide-1)-shift, (tileSide-1)-shift, (tileSide-1)-shift, 
					pointsY, pointsX, sparePointsY, sparePointsX, tileSide, 
					0, true, diagonal, debug);
					for(int i = 0; i<index;++i){
						spareTileMap[(pointsY[i]*tileSide) + pointsX[i] ] = 2;
				...
				// ---
				randLine(rand(), &index, &spareIndex, pushCoefficient, 
				0, (tileSide-1)-shift, (tileSide-1), (tileSide-1)-shift,
				pointsY, pointsX, sparePointsY, sparePointsX, tileSide, 
				0, true, diagonal, debug); //
				for(int i = 0; i<index;++i){
					spareTileMap[(pointsY[i]*tileSide) + pointsX[i] ] = 2;
		...
		if(checkDir[8] > level+1 && checkDir[5] <= level+1 && checkDir[7] <= level+1){ // SE corner - single
			// ---
			randLine(rand(), &index, &spareIndex, pushCoefficient, 
			(tileSide-1)-shift, (tileSide-1), (tileSide-1), (tileSide-1)-shift, 
			pointsY, pointsX, sparePointsY, sparePointsX, tileSide, 
			0, true, diagonal, debug);
			for(int i = 0; i<index;++i){
				spareTileMap[(pointsY[i]*tileSide) + pointsX[i] ] = 2;
