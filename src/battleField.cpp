...
battlefield::battlefield(const tile* parentTInput, const int yInput, const int xInput):
	...
	debug(parentT->debug)
{
	...
	if(height){
		while(amount>0){
			...
			//				);
			battleMap[((rand()%battlefieldSide)*battlefieldSide)+(rand()%battlefieldSide)] = -4;
			amount--;
		}
	}
	if(debug){
