		...
		if(special==0){
			if(goUp){ // dealing with y as input
				...
				if(xIndex != (int)(checkOffset - 0.00001) ){ // 0.00...1 is the inaccuracy, but needed for trunsation. remove both indentY and indentY adding to playerY above.
					...
					xIndex += rightMode;
					indexHolder = func((double)(xIndex+indentX-playerX),(1.00/function),(double)playerY+indentY);
					yIndex = (int)indexHolder;
					currHeight = actualMap[(yIndex*length)+xIndex];
					tempAngle = (currHeight - playerHeight)/distD(playerY,playerX,indexHolder,(double)xIndex); 
					std::cout << "WOOP " << func((double)(xIndex+indentX-playerX),(1.00/function),(double)playerY+indentY) << std::endl;
				...
				if(yIndex != (int)(checkOffset - 0.00001) ){ // 0.00...1 is the inaccuracy, but needed for trunsation. remove both indentY and indentY adding to playerY above.
					//std::cout << "GORE3" << std::endl;
					yIndex += upMode;
					indexHolder = func((double)(yIndex+indentY-playerY),(1.00/function),(double)playerX+indentX);
					xIndex = (int)indexHolder;
					currHeight = actualMap[(yIndex*length)+xIndex];
					tempAngle = (currHeight - playerHeight)/distD(playerY,playerX,(double)yIndex,indexHolder); 
					// removed indents, xdiff and ydif is same anyhow, and that is used for calculating distance.
				}
				else{
					//std::cout << "GORE4" << std::endl;
